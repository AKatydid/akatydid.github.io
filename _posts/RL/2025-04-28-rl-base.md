...
上文中价值迭代和策略迭代都基于一个前提——已知状态转移函数和奖励 $R(s_t, a_t)$。但是，实际上很难对环境进行细致的建模，状态转移和奖励 $R(s_t, a_t)$ 通常是未知的。我们通常把已知状态转移函数和奖励条件下求解最优策略称作 model-base 方法，而未知情况下求解则称作 model-free 方法。
...
* 混合网络：QMIX 核心，是一个前馈神经网络，将所有智能体网络的输出 $Q_a$ 作为输入，并将它们非线性地混合，最终输出团队的联合价值 Q_tot；
